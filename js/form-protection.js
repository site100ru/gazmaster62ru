...
/**
 * Получает правила валидации из формы
 */
function getConfigFromField(field) {
    const form = field.closest('form');
    return form ? form.validationRules : DEFAULT_VALIDATION_RULES;
}
...
function validateRequired(value, field) {
    const config = getConfigFromField(field);

    if (!config.requireAllFields) {
        return { valid: true };
    }
 ...
 */
function validateNameCyrillic(name, field) {
    const config = getConfigFromField(field);

    if (!config.nameOnlyCyrillic || !name || name.trim() === '') {
        return { valid: true };
    }
 ...
 */
function validateEmailLatin(email, field) {
    if (!email || email.trim() === '') {
        return { valid: true };
    }
 ...
 */
function validatePhoneSameDigits(phone, field) {
    if (!phone || phone.trim() === '') {
        return { valid: true };
    }
 ...
 */
function validatePhoneSequentialDigits(phone, field) {
    if (!phone || phone.trim() === '') {
        return { valid: true };
    }
    ...
    if (!config.phoneSequentialDigits) {
        return { valid: true };
    }
 ...
 */
function validateCityCyrillic(city, field) {
    if (!city || city.trim() === '') {
        return { valid: true };
    }
 ...
 */
function validateRussianOperator(phone, field) {
    if (!phone || phone.trim() === '') {
        return { valid: true };
    }
    ...
    if (!config.phoneRussianOperators) {
        return { valid: true };
    }
 ...
 */
function validateHoneypotName(formData, form) {
    const config = form.validationRules || DEFAULT_VALIDATION_RULES;

    if (!config.honeypotName) {
        return { valid: true };
    }
 ...
 */
function validatePhoneFullLength(phone, field) {
    if (!phone || phone.trim() === '') {
        return { valid: true };
    }
 ...
 */
function validateFormTimestamp(timestamp, form) {
    const config = form.validationRules || DEFAULT_VALIDATION_RULES;

    if (!config.formTimestamp) {
        return { valid: true };
    }
...
function validateNameField(field) {
    ...
    let result = validateRequired(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }

    result = validateNameCyrillic(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }
...
function validateEmailField(field) {
    ...
    result = validateEmailLatin(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }
...
function validatePhoneField(field) {
    ...
    result = validatePhoneFullLength(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }

    result = validatePhoneSameDigits(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }

    result = validatePhoneSequentialDigits(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }

    result = validateRussianOperator(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }
...
function validateCityField(field) {
    ...
    const result = validateCityCyrillic(value, field);
    if (!result.valid) {
        showFieldError(field, result.error);
        return false;
    }
...
function validateFormBeforeSubmit(form) {
    ...
    const honeypotResult = validateHoneypotName(formData, form);
    if (!honeypotResult.valid) {
        isValid = false;
    }

    const timestamp = formData.get('form_timestamp') || '';
    const timestampResult = validateFormTimestamp(timestamp, form);
...
function initFormProtection() {
    ...
    forms.forEach(function (form) {
        // Получаем правила для конкретной формы
        const formId = form.getAttribute('id');
        const formRules = getFormValidationRules(formId);
        
        // Создаем функцию для проверки конфига этой формы
        function checkConfig(ruleName) {
            return formRules[ruleName] === true;
        }
        
        // Теперь в validateField и других функциях будут использоваться правила этой формы
        // Сохраняем правила в data-атрибут формы для доступа из других функций
        form.validationRules = formRules;
